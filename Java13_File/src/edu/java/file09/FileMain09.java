...
public class FileMain09 {
	...
	public static void main(String[] args) {
		 ...
		 * 		 
		 * 상대 경로(relative path):
		 * 	현재 작업 디렉토리(CMD)를 기준으로 파일 또는 폴더가 있는 위치를 상대적으로 찾아가는 방식.
		...
		//-> 파일 객체가 생성될 때 상대 경로로 생성된 경우 getPath() 메서드는 상대경로를 리턴.
		//-> 파일 객체를 생성할 때 절대 경로로 생성하면 getPath() 메서드는 절대경로를 리턴.
		...
		// 기존 폴더 삭제: (1)File 객체 생성 (2)폴더가 있으면, delete() 메서드 호출
		if (newFolder.exists()) {
			newFolder.delete();
