 ...
 * 	모든 파일 형식(txt, jpg, mp4, docx, pptx, ...)에 사용 가능.
 * 	InputStream
 * 	|__ FileInputStream, BufferedInputStream, ObjectInputStream, ...
 * 	OutputStream
 * 	|__ FileOutStream, BufferedOutputStream, ObjectOutputStream, ...
 * 
 * Reader/Writer: 문자 스트림(character stream). 읽기/쓰기 기본 단위는 "문자" 단위.
 ...
 * 
 * 인코딩(encoding): 문자에 해당하는 코드(정수)로 변환 'A' -> 65
...
public class FileMain08 {
	...
	public static void main(String[] args) {
		...
		try( 
				...
				// 2. 인코딩을 설정할 문자 스트림을 생성
				InputStreamReader reader = new InputStreamReader(in, "EUC-KR"); 
				// 3. 파일에서 직접 읽는 것보다 메모리에서 읽는것이 빠르기 때문 -> 메모리 버퍼를 사용해서 읽기 속도를 빠르게~!
