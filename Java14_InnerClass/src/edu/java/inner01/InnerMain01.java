 ...
 * 		 - 선언된 위치에서부터 변수가 포함된 블록({...})이 끝날때까지만 사용 가능.
 * 		 - 파라미터 지역 변수의 한 종류.
 ...
 * 		  - 선언된 메서드의 블록안에서만 객체를 생성할 수 있는 클래스.
...
public class InnerMain01 {
	...
	public int x; // 인스턴스 변수 -> 메모리 Heap 영역에 만들어짐 (생성자를 호출 했을 때)
...
//		static int y;
		...
		// 인스턴스는 내부 클래스 참조 변수를 사용해야 생성자를 호출할 수 있음.

//		Outer.Inner inner1 = outer1.new Inner(200); //-> import 문장이 없는 경우
...
//		Enclosing.Nested nested = new Enclosing.Nested(2); //-> import 문장이 없는 경우
