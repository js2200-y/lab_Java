...
// (4) public : 어디서 직접 접근(사용) 가능.
// 가시성(visibility) 범위 : private < (package) < protected < public
//  수식어가 없는 경우 같은 패키지에서만 사용가능, 퍼블릭은 어디서나
...
public class ModifierMain01 {
	...
	//	acc.balance = 100; //-> not visible: 수식어가 없는 경우 다른 패키지에서는 보이지 않음.
		acc.printInfo(); // public으로 공개된 메서드는 어디서든 사용 가능.
		
		// 같은 패키지에 있는 클래스 타입은 import 문장 없이 사용 가능.
		AccessTest test = new AccessTest(1, 2, 3, 4);
//		System.out.println(test.a); // 에러이유: not visible - private이기 때문
		System.out.println(test.b); // 수식어가 없는 경우 같은 패키지 범위에서는 보임.
		test.printInfo(); // 퍼블릭이란 메서드를 통해 간접적으로 볼수 있음.
		
//		test.a = 100; // 에러이유: not visible - private이기 때문
		test.d = 100;
		test.printInfo();
