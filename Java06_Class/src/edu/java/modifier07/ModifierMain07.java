...
// - (JRE)Java Runtime Environment 사용하는 메모리 공간 중에서 힙(heap)에 생성됨. 

// static 멤버(필드, 메서드):
// - static 수식어 사용된 멤버
// - 객체를 생성하지 않고 사용할 수 있는 멤버.
// - 프로그램 로딩 시점(main 메서드 시작 전) JRE이 사용하는 메모리 공간 중에서 메서드(method)
// - 클래스 이름을 사용해서 사용하는 멤버.
// - static 메서드는 static 멤버(필드, 메서드)만 사용 가능.
// ->  static 메서드는 인스턴스 멤버(필드, 메서드)를 사용할 수 없음.


public class ModifierMain07 {
	
	public static void test1() { // 객체 생성 없이 사용 가능
		System.out.println("test 1");
	}

	public void test2() { // 객체 생성 후 사용 가능
	...
	public static void main(String[] args) {
		...
		test1(); // 같은 클래스 안에 있어서 앞에 수식어 생략 가능.
