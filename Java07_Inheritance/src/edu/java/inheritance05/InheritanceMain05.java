...
import edu.java.inheritance06.Point;

// final: 변경할 수 없는
// final 지역변수 : 값을 한번 초기화하면 그 값을 변경할 수 없는 변수(상수, constant)
// final 필드 : 반드시 명시적으로 값을 초기화해야 하고, 초기화 된 이후에는 값을 변경할 수 없는 필드.
//				(1) final 필드를 선언할 때 초기화 하거나, 또는
//				(2) final 필드를 초기화하는 생성자가 반드시 있어야 함.
// final 메서드 : override할 수 없는 메서드. 하위 클래스에서 재정의 할수 없도록 선언한 메서드
...
//	public void test2() {} -> final 메서드는 override 할수 없는 메서드.
...
public class InheritanceMain05 {
	...
	private final int y; // 생성자에서 명시적으로 초기화하는 코드가 있어야함.
