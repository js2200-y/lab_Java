...
public class ThreadMain02 {
	...
	public static void main(String[] args) {
		 ...
		 *  자바는 인터페이스구현은 개수 제한이 없음.
		 *    class A extends B implements C, D, E {...} -> 가능~!
		 *    
		 *  Thread 사용방법 2:
		 *  1. Runnable 인터페이스를 구현하는 클래스를 선언(정의)
		 *  2. 구현 클래스에서 Runnable 인터페이스의 run 메서드를 재정의 - 쓰레드가 할일~!
		 *  3. Thread 생성자를 호출할 때 Runnable 구현 객체를 argument로 전달.
		 *    new Thread(Runnable runnable) 생성자 호출.
		 *  4. 쓰레드 객체의 start 메서드 호출 -> 쓰레드 실행 -> run 자동실행
		 */
