...
//		int max = arr[0]; // 배열의 최대값을 저장할 변수.
		int index = 0; // 배열의 최대값의 인덱스를 저장할 변수.
		for (int i= 0; i < arr.length; i++) { //배열의 원소들을 순서대로 반복하면서
			if (arr[i] > arr[index]) { //배열에서 읽은 값이 max보다 크다면
//			max = arr[i]; // max 값을 배열에서 읽은 값으로 변경
				index = i; // 최대값의 인덱스를 현재 인덱스로 변경
			}
		}
		return index;
	}
...
//				minVal = x;
	...
	public static int minAt(int[] arr) {
	 int minVal = min(arr); 	// 최소값을 찾음.
	 int index = 0; // 배열의 인덱스
	 for(index = 0; index < arr.length; index++) {
		// 배열의 원소들을 순서대로 반복하면서, 그 원소가 최소값인지 검사.
		 if (arr[index] == minVal)
			 break; // 최소값을 찾았으면 반복을 종료.
	 }
