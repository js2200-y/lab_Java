 ...
 * List, Set의 공통점: 한가지 타입의 객체들을 여러개 저장 할 수 있는 데이터 타입.
 * List: 인덱스 있음. 저장하는 순서가 중요. (add 호출 할 때 마다 인덱스 자동 증가). 중복된 값을 저장할 수 있음.
 ...
 * 			리스트에서 인덱스의 역할 (데이터를 저장/검색/수정/삭제 할때 값을 찾기 위한 용도).
...
public class MapMain01 {
	...
	public static void main(String[] args) {
		...
		// key 매핑된 value를 리턴. key에 매핑된 value가 없으면 defalutValue를 리턴.
		...
		// put(key, value):
		// (1) Map에 key가 존재하지 않으면key-value 데이터를 저장.
		// (2) Map에 key가 존재하면 key에 매핑된 value를 변경.
		students.put(3, "Kim Dohyun");
		System.out.println(students);
		
		// Map에서 향상된 for 문장을 사용하는 방법:
		Set<Integer> keySet = students.keySet(); // (1) Map의 key들로만 이루어진 Set을 만듬.
